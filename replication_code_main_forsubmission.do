...
*= TABLE 2: Summary Statistics =================================================
   ...
   local tab2vars married numkids hasland sharecrops anymissedmeal30 		 ///
				  nofindwork30 bl_numdaysworked bl_numdaysworked_invill 	 ///
				  bl_totwage endlinewage piecerate_exp baselineprod baselineatt
   
*  Overall sample
   sum `tab2vars' if day_centered==0
...
*  Panel B: Cross-unit comparisons
   ...
   keep round C1 C2 C3 Het uid task_id team_id dk_agarbati dk_wick 			 ///
   dk_leafmat dk_paperbag dk_all_agarbati dk_all_wick dk_all_leafmat 		 ///
   dk_all_paperbag agarbati_correct wick_correct leafmat_correct 			 ///
   paperbag_correct treat_agarbati treat_wick treat_leafmat treat_paperbag
   ...
   keep correct_1 correct_2 correct_3 correct_4 treat_1 treat_2 treat_3 	 ///
   treat_4 dk_1 dk_2 dk_3 dk_4 dk_all_1 dk_all_2 dk_all_3 dk_all_4 round uid ///
   team_id task_id
...
*  Drop if taskq is the same task as a person is on
   ...
   local tab3vars_b correct_ not_dk_ not_dk_all_
   
   bysort teamHet: sum `tab3vars_b'
...
*  Panel A: Pooled Treatment Effects
   ...
   global tab4actrls_nofe posttreat lowpost medpost highpost irrelpostlow 	 ///
						  irrelpostmed irrelposthigh low_p med_p high_p		 ///
						  treatlow treatmed treathigh irrellow irrelmed		 ///
						  irrelhigh i.day_round exper_task_lin*				 ///
						  exper_task_sq* i.task_id $neighbor_all
   
   global tab4actrls_fe  posttreat lowpost medpost highpost irrelpostlow 	 ///
						 irrelpostmed irrelposthigh $neighbor_post_noT 		 ///
						 $neighbor_post i.day_round exper_task_lin* 		 ///
						 exper_task_sq*
   
   // without FE
   qui reg prodnorm `tab4actrls_nofe', cl(team_id)
   eststo reg1
   
   qui reg attendance `tab4actrls_nofe', cl(team_id)
   est sto reg2
   
   // with FE
   qui xtreg prodnorm `tab4actrls_fe', fe cl(team_id)
   est sto reg3
   
   qui xtreg attendance `tab4actrls_fe', fe cl(team_id)
   est sto reg4
   
   // condl on attendance - no FE
   qui reg prodnorm `tab4actrls_nofe' if attendance==1, cl(team_id)
   est sto reg5 
   
   esttab reg1 reg3 reg2 reg4 reg5, stats(r2 N, labels("R-squared" "N")) 	 ///
		  se(3) replace starlevels(* 0.10 ** 0.05 *** .01) keep(posttreat)
   
*  Panel B: Treatment Effects Separately by Rank
   
   global tab4bctrls_nofe treatlowpost treatmedpost treathighpost lowpost 	 ///
						  medpost highpost irrelpostlow irrelpostmed 		 ///
						  irrelposthigh low_p med_p high_p treatlow treatmed ///
						  treathigh irrellow irrelmed irrelhigh  i.day_round ///
						  exper_task_lin* exper_task_sq* i.task_id $neighbor_all
						  
   global tab4bctrls_fe   treatlowpost treatmedpost treathighpost lowpost 	 ///
						  medpost highpost irrelpostlow irrelpostmed 		 ///
						  irrelposthigh $neighbor_post_noT $neighbor_post 	 ///
						  i.day_round exper_task_lin* exper_task_sq*
   
   // without FE
   qui reg prodnorm `tab4bctrls_nofe', cl(team_id)
   est sto reg6
   
   qui reg attendance `tab4bctrls_nofe', cl(team_id)
   est sto reg7
   
   // with FE
   qui xtreg prodnorm `tab4bctrls_fe', fe cl(team_id)
   est sto reg8
   
   qui xtreg attendance `tab4bctrls_fe', fe cl(team_id)
   est sto reg9
   
   // condl on attendance - no FE
   qui reg prodnorm `tab4bctrls_nofe' if attendance==1, cl(team_id)
   est sto reg10 
   
   sum prodnorm attendance if post==1 & Het==0 & relevant==1
   sum prodnorm if post==1 & Het==0 & relevant==1 & attendance==1
   
   esttab reg6 reg8 reg7 reg9 reg10, stats(r2 N, labels("R-squared" "N")) 	 ///
   se(3) replace starlevels(* 0.10 ** 0.05 *** .01) keep(treatlowpost 		 ///
   treatmedpost treathighpost)
...
*  Panel A: Pooled Treatment Effects
   ...
   global tab5actrls posttreat lowpost medpost highpost irrelpostlow 		 ///
					 irrelpostmed irrelposthigh $neighbor_post_noT 			 ///
					 $neighbor_post i.day_round exper_task_lin* exper_task_sq*
   
   // Before first payday
   qui xtreg prodnorm `tab5actrls' if (day_centered<first_post_pay), fe cl(team_id)   
   est sto treg1p
   
   qui xtreg attendance `tab5actrls'  if (day_centered<first_post_pay), fe cl(team_id) 
   est sto treg2p
   
   // After first, before second payday
   qui xtreg prodnorm `tab5actrls' if (day_centered<0 | (day_centered>=first_post_pay & day_centered<second_post_pay)), fe cl(team_id)   
   est sto treg3p
   
   qui xtreg attendance `tab5actrls'  if (day_centered<0 | (day_centered>=first_post_pay & day_centered<second_post_pay)), fe cl(team_id)  
   est sto  treg4p
   
   // After second payday
   qui xtreg prodnorm `tab5actrls' if (day_centered<0 | (day_centered>=second_post_pay)), fe cl(team_id)   
   est sto  treg5p
   
   qui xtreg attendance `tab5actrls' if (day_centered<0 | (day_centered>=second_post_pay)), fe cl(team_id) 
   est sto  reg6p
   
   esttab treg1p treg3p treg5p treg2p treg4p treg6p, stats(r2 N, 			 ///
   labels("R-squared" "N")) se(3) replace starlevels(* 0.10 ** 0.05 *** .01) ///
   keep(posttreat)
...
*= TABLE 10: Effects on Group Cohesion: Endline Games 2 ========================
   ...
   qui reg num_correct same_team_het same_team oneHet both_primary_sch 		 ///
					   both_writesent_oriya   pair_any_relteam i.gameorder 	 ///
					   i.station_no, cluster(gameround) 
   eststo labg2_1
   
   qui reg num_correct same_team_het same_team oneHet anyR2 anyR3 			 ///
					   both_primary_sch both_writesent_oriya   				 ///
					   pair_any_relteam i.gameorder i.station_no, 			 ///
					   cluster(gameround) 
   eststo labg2_2
   
   qui reg num_correct same_team_het same_team oneHet 						 ///
					   same_team_het_justified oneHet_justified   			 ///
					   both_primary_sch both_writesent_oriya 				 ///
					   pair_any_relteam anyR2 anyR3 i.gameorder 			 ///
					   i.station_no, cluster(gameround) 
   eststo labg2_3
   ...
   qui reg num_correct same_team_het same_team oneHet 						 ///
					   same_team_het_justified oneHet_justified 			 ///
					   both_primary_sch both_writesent_oriya 				 ///
					   pair_any_relteam r11 r12 r13 r22 r23 i.gameorder 	 ///
					   i.station_no, cluster(gameround) 
   eststo labg2_4
   ...
   esttab labg2_1 labg2_2 labg2_3 labg2_4, stats(r2 N, 						 ///
   labels("R-squared" "N")) se(3) replace starlevels(* 0.10 ** 0.05 *** .01) ///
   keep(same_team_het same_team oneHet anyR2 anyR3 same_team_het_justified oneHet_justified)
...
*= Table 11: Social Cohesion Outside Work: Network Formation ===================
   ...
   esttab net1 net2 net3 net4, stats(r2 N, labels("R-squared" "N")) se(3) 	 ///
   replace starlevels(* 0.10 ** 0.05 *** .01) keep(Het _IHet_1 _IHetXobspr_1 _cons)
...
*= OA Table 1: Summary Statistics by Rank  =====================================
...
*= OA Table 2: Effects of Pay Disparity: Robustness to Alternate Specifications
...
*= OA Table 3: Effects on Output Quality ==========================
*  Panel A - Pooled
...
*= OA Table 4: Perceived Justifications: Robustness of Relative Productivity Results
...
* OA Table 5: Perceived Justifications: Robustness to Alternative Cutoffs for Relative Productivity Thresholds
   * 10%   
...
*= OA Table 6: Perceived Justifications: Task Observability Robustness =========
...
*= OA Table 7: Absence of Correlation Between Task Observability Measure and Training Outcomes
...
*= OA Table 8: Sample Overlap Between Perceived Justification Measures =========
...
*= OA Table 10: Robustness: Effects on Group Cohesion - Conditional on Attendance
...
*= OA Table 11: Effects of Pay Disparity by Village Ties =======================
*  Panel A — Pooled Treatment Effects
...
*= OA Table 12: Lack of Evidence of Attendance Peer Effects ====================
...
*= OA Table 13: Response to Disclosure of Productivity Ranks in Training Period
   xtset uid
...
*  OA Figure 1: Stability of Relative Productivity Ranks Across Time ===========
...
*  OA Figure 2: Total Attendance for Pay Disparity and Compressed Units ========
...
*  OA Figure 3: Density of Raw Production by Task, Scaled by Mean ==============
   forvalues i=1/10 {
...
*  OA Figure 5: Distribution of Village Prevailing Wages: Endline Survey Responses
